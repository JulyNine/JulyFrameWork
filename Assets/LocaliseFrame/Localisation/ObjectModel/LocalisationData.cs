...
namespace GameFramework.Localisation.ObjectModel
{
    ...
    public class LocalisationData : ScriptableObject , ISerializationCallbackReceiver
    {
        ...
        public string GetText(string key, string language)
        {
            ...
            if (languageIndex == -1)
            {
                //Debug.Log("Localisation key {0} not found for language {1}.", key, language);
                return null;
            }
        ...
        public string GetText(string key, int languageIndex)
        {
            ...
            if (entry == null)
            {
                //Debug.Log("Localisation key {0} not found for language {1} (index {2}).", key, Languages[languageIndex].Name, languageIndex);
                return null;
            }
